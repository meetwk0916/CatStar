#!/usr/bin/env python3
"""Extract the product-action v3 bowl-aligned eating sheet."""
...
FRAME = 96
OUT_DIR = ARTIFACTS_ART_ROOT / "candidates" / "active" / "product-cat-actions-v3"
SOURCE = OUT_DIR / "sources" / "eat-bowl-aligned-source.png"
...
def normalize(subject: Image.Image) -> tuple[Image.Image, dict[str, object]]:
    ...
    if resized.height > 78:
        scale = 78 / resized.height
        resized = resized.resize(
            (max(1, round(resized.width * scale)), 78),
            Image.Resampling.LANCZOS,
        )

    frame = Image.new("RGBA", (FRAME, FRAME), (0, 0, 0, 0))
    x = (FRAME - resized.width) // 2
    y = FRAME - resized.height - 8
...
def main() -> None:
    ...
    if len(groups) != 6:
        raise SystemExit(f"Expected 6 eating frames, found {len(groups)} groups: {groups}")
    ...
    for index, (start, end) in enumerate(groups, start=1):
        ...
        frames.append(frame)
        frame.save(NORMALIZED_DIR / f"eat-{index:02d}.png")
        metadata.append({"frame": index, "source_x": [start, end], **info})

    sheet = make_sheet(frames)
    sheet.save(SHEET_DIR / "eat.png")

    preview = Image.new("RGBA", (FRAME * len(frames), FRAME), (24, 24, 24, 255))
    preview.alpha_composite(sheet, (0, 0))
    preview.save(SHEET_DIR / "eat-preview.png")

    (OUT_DIR / "metadata.json").write_text(json.dumps({"eat": metadata}, indent=2) + "\n")
    (OUT_DIR / "README.md").write_text(
        "# Product Cat Actions v3\n\n"
        "Bowl-aligned eating candidate generated as a six-frame chroma-key source and normalized to the CatStar `96x96` action contract.\n\n"
        "This replaces the first eating candidate with a clearer lower-head sniff/eat loop that aligns better with the existing food bowl prop.\n",
    )
    print(f"Wrote {SHEET_DIR / 'eat.png'}")
