#!/usr/bin/env python3
"""Extract the product-action v5 natural jump sheet."""
...
FRAME = 96
OUT_DIR = ARTIFACTS_ART_ROOT / "candidates" / "active" / "product-cat-actions-v5"
SOURCE = OUT_DIR / "sources" / "jump-natural-source.png"
...
def normalize(subject: Image.Image, index: int) -> tuple[Image.Image, dict[str, object]]:
    ...
    # Keep the anticipation/landing frames low and the airborne frames centered
    # within the sprite so the scripted path controls world-space height.
    vertical_offsets = {
        1: 1,
        2: -2,
        3: -4,
    ...
    y = FRAME - resized.height - 7 + vertical_offsets.get(index, 0)
...
def main() -> None:
    ...
    if len(groups) != 5:
        raise SystemExit(f"Expected 5 jump frames, found {len(groups)} groups: {groups}")
    ...
    (OUT_DIR / "metadata.json").write_text(json.dumps({"jump": metadata}, indent=2) + "\n")
    (OUT_DIR / "README.md").write_text(
        "# Product Cat Actions v5\n\n"
        "Natural jumping candidate generated as a five-frame chroma-key source and normalized to the CatStar `96x96` action contract.\n\n"
        "This replaces the deterministic v2 jump stabilization with a dedicated crouch, launch, airborne, landing, and landing-crouch sequence.\n",
    )
