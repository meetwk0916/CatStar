#!/usr/bin/env python3
"""Extract the product-action v8 deep sleep sheet."""
...
FRAME = 96
OUT_DIR = ARTIFACTS_ART_ROOT / "candidates" / "active" / "product-cat-actions-v8"
SOURCE = OUT_DIR / "sources" / "sleep-deep-source.png"
...
def main() -> None:
    ...
    if len(groups) != 4:
        raise SystemExit(f"Expected 4 sleep frames, found {len(groups)} groups: {groups}")
    ...
    for index, (start, end) in enumerate(groups, start=1):
        ...
        frames.append(frame)
        frame.save(NORMALIZED_DIR / f"sleep-{index:02d}.png")
        metadata.append({"frame": index, "source_x": [start, end], **info})

    sheet = make_sheet(frames)
    sheet.save(SHEET_DIR / "sleep.png")

    preview = Image.new("RGBA", (FRAME * len(frames), FRAME), (24, 24, 24, 255))
    preview.alpha_composite(sheet, (0, 0))
    preview.save(SHEET_DIR / "sleep-preview.png")

    (OUT_DIR / "metadata.json").write_text(json.dumps({"sleep": metadata}, indent=2) + "\n")
    (OUT_DIR / "README.md").write_text(
        "# Product Cat Actions v8\n\n"
        "Deep sleeping candidate generated as a four-frame chroma-key source and normalized to the CatStar `96x96` action contract.\n\n"
        "This separates explicit deep sleep from the awake `lie` companionship loop.\n",
    )
    print(f"Wrote {SHEET_DIR / 'sleep.png'}")
