...
v3 is a direct redraw from the approved rounded short-haired model sheet and
the v11 `sit` and `walk` identity references. It does not use v1 or v2 as an
edit target. The four right-facing poses keep a low, broad, deep-bodied adult
silhouette, short sturdy legs, wide cheeks, a short neck, and a tail close to
the hindquarters. Motion remains limited to breathing, one slow blink, and a
small close-tail shift.
